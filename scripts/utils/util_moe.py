...
import numpy as np 
class MOE :
    def __init__(self, port_name='/dev/ttyUSB0', baudrate=57600, protocol_version=2.0, servo_ids=[0,1,2,3,4,5], override_initial_positions=None):
        ...
        for dxl_id in self.DXL_IDs:
            self.packetHandler.write1ByteTxRx(self.portHandler, dxl_id, 64, self.TORQUE_ENABLE)
    ...
    def return_to_initial_positions(self):
        ...
        """
        self.smooth_move_to_positions(self.initial_positions, delay=-1, threshold=10)

    def disable_torque(self):
        for dxl_id in self.DXL_IDs:
            self.packetHandler.write1ByteTxRx(self.portHandler, dxl_id, 115, self.TORQUE_DISABLE)
...
if __name__ == "__main__":
    ...
    try:
        while True:
            target_positions = [-10, -20, -10, -10, -10, -10]  # Example positions for each servo
            hand.smooth_move_to_positions(target_positions)
            time.sleep(2)  # Pause before next movement
            target_positions = [10, 20, 10,10, 10, 10]  # Move back to initial positions
            hand.smooth_move_to_positions(target_positions)
            time.sleep(2)  # Pause before next movement
